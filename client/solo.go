...
func readResponse(reader *bufio.Reader) (interface{}, error) {
	...
	if strings.HasPrefix(line, "-ERR") {
		errmesg := strings.TrimSpace(line[4:])
		return nil, fusionError(errmesg)
	}

	if line[0] == ':' {
		n, err := strconv.ParseInt(strings.TrimSpace(line[1:]), 10, 64)
		if err != nil {
			return nil, fusionError("Int reply is not a number")
		}
		return n, nil
	}
	...
	if line[0] == '*' {
		...
		if err != nil {
			return nil, fusionError("MultiBulk reply expected a number")
		}
		if size <= 0 {
...
func (client *Client) openConnection() (c net.Conn, err error) {
	...
	c, err = net.DialTimeout("tcp", addr, 30*1e9)
	if err != nil {
		return
	}
...
func (client *Client) sendCommands(cmdArgs <-chan []string, data chan<- interface{}) (err error) {
	...
	if err == io.EOF {
		...
		if pong != "PONG" {
			return fusionError("Unexpected response to PING.")
		}
		if err != nil {
			goto End
		}
...
func (client *Client) Get(key string) ([]byte, error) {
	...
	if res == nil {
		return nil, fusionError("Key `" + key + "` does not exist")
	}
...
func (client *Client) Setnx(key string, val []byte) (bool, error) {
	...
	return false, fusionError("Unexpected reply to SETNX")
}
...
func (client *Client) Msetnx(mapping map[string][]byte) (bool, error) {
	...
	return false, fusionError("Unexpected reply to MSETNX")
}
...
func (client *Client) Spop(key string) ([]byte, error) {
	...
	if res == nil {
		return nil, fusionError("Spop failed")
	}
...
func (client *Client) Hsetex(key string, field string, ex int, val []byte) (bool, error) {
	res, err := client.sendCommand("HSETEX", key, field, strconv.Itoa(ex), string(val))
	if err != nil {
		return false, err
	}
...
func (client *Client) Hget(key string, field string) ([]byte, error) {
	...
	if res == nil {
		return nil, fusionError("Hget key or failed not exists")
	}
...
func (client *Client) Hgetall(key string, val interface{}) error {
	...
	if data == nil || len(data) == 0 {
		return fusionError("Key `" + key + "` does not exist")
	}
