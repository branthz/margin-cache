...
// Modified from https://github.com/stathat/consistent
package proxy 
...
func New() *Consistent {
	...
	c.members = make(map[string]Target)
	return c
}
...
func (c *Consistent) GetTarget(name string) (Target, error) {
	...
	key := c.hashKey(name)
	i := c.search(key)
	sv:=c.circle[c.sortedHashes[i]]
	return c.members[sv],nil
}
