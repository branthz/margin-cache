...
var doesNotExist = marginError("Key does not exist ")
...
func readBulk(reader *bufio.Reader, head string) ([]byte, error) {
	...
	default:
		return nil, marginError("Expecting Prefix '$' or ':'")
	}
...
func readResponse(reader *bufio.Reader) (interface{}, error) {
	...
	if strings.HasPrefix(line, "-ERR") {
		...
		return nil, marginError(errmesg)
	}

	if line[0] == ':' {
		n, err := strconv.ParseInt(strings.TrimSpace(line[1:]), 10, 64)
		if err != nil {
			return nil, marginError("Int reply is not a number")
		}
		return n, nil
	}
	...
	if line[0] == '*' {
		...
		if err != nil {
			return nil, marginError("MultiBulk reply expected a number")
		}
		if size <= 0 {
...
func (client *Client) sendCommands(cmdArgs <-chan []string, data chan<- interface{}) (err error) {
	...
	if err == io.EOF {
		...
		if pong != "PONG" {
			return marginError("Unexpected response to PING.")
		}
		if err != nil {
			goto End
		}
...
func (client *Client) Get(key string) ([]byte, error) {
	...
	if res == nil {
		return nil, marginError("Key `" + key + "` does not exist")
	}
...
func (client *Client) Spop(key string) ([]byte, error) {
	...
	if res == nil {
		return nil, marginError("Spop failed")
	}
...
func (client *Client) Hget(key string, field string) ([]byte, error) {
	...
	if res == nil {
		return nil, marginError("Hget key or failed not exists")
	}
...
func (client *Client) Hgetall(key string, val interface{}) error {
	...
	if data == nil || len(data) == 0 {
		return marginError("Key `" + key + "` does not exist")
	}
